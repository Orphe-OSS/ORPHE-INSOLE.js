...
6. Realtime Format 4 + Step Analysis
...
現行FWではFIFO read mode中にStep Analysisが配信されず、短時間Realtimeへ戻すとその区間の
FIFOデータがFW側で`no-data`になります。このためlossless FIFOとStep Analysisは排他とし、
FIFOは保存計測、Realtime + Step Analysisはアプリ向け同時利用として分けます。
...
7. Realtime Format 4 + Step Analysisを選び、RawのQuaternionを含む全フィールドと
   Step notifyが同じ計測区間で継続することを確認する。アプリでRaw表示とStep判定を
   同時利用する場合はこの構成を使う。
8. 別Macでは同じHostラベル以外の条件を揃えて、単体FIFO、2台FIFOを再現する。
   FIFOとStepは同時実行せず、必要ならFIFO停止・drain完了後にRealtime + Stepへ切り替える。
