...
- Public `examples/data-modes/` interactive guide and recorder, promoted from the manual validator. It explains Realtime Notify, FIFO request-response, and Step Analysis from a first-time user's goals; uses the same named profile/measurement API shown in its copyable code; compares realtime formats 1/3/4, FIFO continuity/loss/drain, Step-only, Realtime Full + Step, left/right devices, and reconnect restoration; and exports the formal measurement window as JSON, Raw CSV, or Step CSV. The old manual URL remains as a compatibility redirect.
