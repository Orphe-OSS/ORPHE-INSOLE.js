...
- Opt-in FIFO/Step cooperative realtime windows (`realtimeWindowMs`, interval, and open/closed callback). `InsoleToolkitSession` enables them only while FIFO and Step Analysis are selected, briefly restores the prior realtime read mode, refreshes STEP_ANALYSIS, then returns to FIFO without stopping the firmware monitor. The manual validator enables 400 ms windows for a real-device compatibility experiment and logs their count; ordinary FIFO collection remains unchanged.
...
- Local-only real-device validation dashboard under `tests/manual/toolkit-mode-validation/` for comparing realtime formats 1/3/4, FIFO continuity/loss/drain, Step-only, FIFO + Step, left/right devices, and reconnect restoration. It separates one-device FIFO baselines from multi-device host-load runs, records a user-provided host label/platform in logs and JSON for Mac-to-Mac comparison, and shows per-device signals, delivery timing, serial continuity, latest decoded records, pass/warn checks, and downloadable evidence without adding a development-only page to the public examples gallery.
...
- `InsoleToolkitSession`: refresh STEP_ANALYSIS subscription across FIFO raw read-mode changes without clearing incomplete or completed gait rows. Real-device validation still treats zero Step packets after a successful refresh as a distinct firmware/read-mode or host-load compatibility failure rather than claiming that re-subscription alone restores publishing.
