...
- Local-only real-device validation dashboard under `tests/manual/toolkit-mode-validation/` for comparing realtime formats 1/3/4, FIFO continuity/loss/drain, Step-only, Realtime Format 4 + Step, left/right devices, and reconnect restoration. It separates one-device FIFO baselines from multi-device host-load runs, records a user-provided host label/platform in logs and JSON for Mac-to-Mac comparison, and shows per-device signals, delivery timing, serial continuity, latest decoded records, pass/warn checks, and downloadable evidence without adding a development-only page to the public examples gallery.
