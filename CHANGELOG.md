...
- `examples/gait-analysis-validation/` — Mac Chrome/Edge + one pair of ORPHE INSOLEs engineering validation page for PR #50. It runs eight BLE-free decoder/lifecycle regression checks, two start/stop smoke cycles, timed straight-walk capture, and a dedicated 60-second disconnect/reconnect test that requires post-reconnect SENSOR_VALUES, STEP_ANALYSIS packets, and completed gait rows. JSON/raw/row CSV and PR-ready Markdown evidence can be exported locally.
