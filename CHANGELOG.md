...
- `InsoleToolkitSession`: keep FIFO + Step Analysis alive across raw read-mode changes. Some devices stop publishing STEP_ANALYSIS after entering or leaving FIFO even though the BLE notification remains subscribed; the session now refreshes that notification after the FIFO transition without clearing incomplete or completed gait rows.
