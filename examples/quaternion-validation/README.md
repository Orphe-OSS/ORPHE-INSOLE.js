...
- 低角速度かつ加速度ノルムが1g付近の区間で、(a) 3軸gyroバイアスをroll/pitchに応じてyawへ投影する方式と、(b) quaternion yawの実測ドリフト率を直接学習する方式を同時比較（ページ内実験のみ）
...
- 既知角度回転に対するquaternion yaw、2方式の適応補正yaw、デバイス時計gyro積分の誤差（gyro物理スケールをその場で確認）
- 周回歩行のquat yaw、2方式の適応補正yaw、生gyro_z積分、実周回角度の比較
...
3軸バイアスは現在のroll/pitchを使ってEuler yaw方向の角速度へ変換し、その積分値だけを生yawから差し引きます。同時に、静止区間のunwrapped yaw変化量から実測yawドリフト率を学習する補正も計算します。CSVには両方式のyawバイアスレート、累積補正量、補正yawを追記します。
