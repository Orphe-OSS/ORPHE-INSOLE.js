...
(function () {
    const translations = {
        ja: {
            ...
            navNext: 'Examples',
            navBuy: '購入',
            headerHomeTitle: 'ORPHE-INSOLE.js トップページ（examples一覧）へ戻る',
            ...
            connectBody: '接続するとヘッダに実測周波数・<span class="badge bg-success">L</span>/<span class="badge bg-primary">R</span> バッジ（装着位置から自動判定）・バッテリーが表示され、ページ内の左右パネルも実際の足に合わせて並び替わります。 <i class="bi bi-gear"></i> ギアアイコンから<strong>Raw Sensor Data / Step Analysis</strong>と Raw Data の<strong>Realtime / FIFO</strong>、Realtime の配信フォーマットを切り替えられます。',
            ...
            connectPrepItem3: '圧力値を見る場合は、Realtime のモード3/4、または FIFO を選びます。',
            modeDataHeader: 'データ',
            modeRateHeader: '周波数',
            modeUseHeader: '用途',
            mode1Use: '姿勢のみ高速取得',
...
insole.gotConvertedGyro = function (gyro) {
  // {x, y, z} 単位: deg/s
            ...
            swapPitchRoll: 'pitch / roll を入れ替えて表示',
            swapPitchRollNote: 'FW・IMU実装によっては pitch と roll の割り当てが入れ替わり、つま先上げが横倒しに見えることがあります。実機の動きと表示が一致しない場合はこのスイッチをONにしてください（3Dモデルと右のEulerゲージの両方に適用。CSVはSDKの生値のまま記録されます）。',
...
insole.gotEuler = function (euler) {
  // {pitch, roll, yaw} 単位: rad
            ...
            fifoTitle: 'ロスレス収録（FIFO）',
            fifoDesc: 'ファームウェアのバッファから<strong>欠損なく</strong>センサーデータを回収します（<code>read_sensor_data_by_tokoroten_loop</code> 相当）。取りこぼしたパケットは自動で再要求されます。収録中はリアルタイム表示が一時停止し、収録データで可視化されます。<strong>FIFOモードにクォータニオンは含まれません</strong>（ジャイロ・加速度・6ch圧力のみ）。',
            fifoStart: '収録開始',
            fifoStop: '収録停止',
            fifoStartHtml: '<i class="bi bi-play-fill"></i> 収録開始',
            fifoStopHtml: '<i class="bi bi-stop-fill"></i> 収録停止',
            fifoDownload: 'CSV保存',
            fifoAutoStop: '欠損したら自動停止',
            fifoStatusStopped: '⚠ 欠損を検知したため自動停止しました: {packets} パケット / 欠損 {dropped} 件 — CSV保存できます',
            fifoStatusIdle: '実機を接続すると使用できます',
            fifoStatusReady: '接続済み — 収録できます',
            fifoStatusPreparing: '準備中…（バッファをクリア）',
            fifoCollecting: '収録中: {packets} パケット / {seconds}秒（追従遅れ {lag}）',
            fifoLossWarning: '⚠ 欠損 {dropped} 件 — BLE追従が間に合いませんでした',
            fifoStatusDone: '収録完了: {packets} パケット / 欠損なし — CSV保存できます',
            fifoStatusDoneLoss: '⚠ 収録完了: {packets} パケット / 欠損 {dropped} 件 — CSV保存できます',
            fifoDraining: '停止後の回収中…（{seconds}秒）— 未回収シリアルをFWバッファから回収しています',
            fifoDrainNote: ' / 停止後回収 {recovered}',
            fifoFailed: '収録に失敗しました（接続を確認してください）',
            gaitTitle: '歩容解析（Gait Analysis）',
            gaitDesc: 'ファームウェアが歩行中にリアルタイム計算する<strong>歩容パラメーター</strong>（ストライド・立脚期/遊脚期・接地パターン・プロネーション等）を取得します。専用の characteristic を 50Hz で subscribe するだけで、1歩ごとに集約された値が届きます（<code>read_gait_analysis</code> 相当）。歩いて（その場足踏みでも可）ください。',
            gaitStart: '計測開始',
            gaitStop: '計測停止',
            gaitStartHtml: '<i class="bi bi-play-fill"></i> 計測開始',
            gaitStopHtml: '<i class="bi bi-stop-fill"></i> 計測停止',
            gaitDownload: 'CSV保存',
            gaitStatusIdle: '実機を接続すると使用できます',
            gaitStatusReady: '接続済み — 歩いて計測できます',
            gaitStatusRunning: '計測中… 歩いてください（{steps} 歩）',
            gaitStatusStopped: '計測完了: {steps} 歩 — CSV保存できます',
            gaitStatusError: 'Step Analysisを開始できませんでした（{codes}）。再接続して再試行してください',
            gaitWaiting: '歩行を待っています…（その場足踏みでも可）',
            gaitColGait: '歩容',
            gaitColStride: 'ストライド',
            gaitColCadence: 'ケイデンス',
            gaitColSpeed: '速度',
            gaitColStrike: '接地',
            gaitColPronation: 'プロネーション',
            gaitColDistance: '総距離',
            gaitColSteps: '歩数',
            nextTitle: '次のステップ（Examples）',
            ...
            nextStepAnalysisBody: 'FWが計算する歩容パラメーターの詳細可視化',
            nextGaitReportBody: '歩行計測レポートの自動生成',
            nextFifoGuideBody: '欠損のないロスレス収録の入門ガイド',
            nextBalanceSwayBody: '重心動揺（CoP軌跡）の可視化・記録',
            nextSonifierBody: 'Web Audio による動作の可聴化',
            nextTerminalBody: '生データ・BLEプロトコルのデバッグ',
            nextAllExamples: 'すべての examples を見る（index）',
            backToIndex: 'index（examples一覧）に戻る',
            relatedLabel: '関連 examples',
            relatedUdonDesc: '圧力で遊ぶ足踏みゲーム（キャリブレーション不要）',
            relatedBalanceTunerDesc: '荷重バランスの可聴化',
            relatedMusicShoeDesc: 'IMUジェスチャで演奏する楽器',
            relatedSonifierDesc: '動作の可聴化（Web Audio）',
            relatedBalanceSwayDesc: '重心動揺（CoP軌跡）の本格的な可視化・記録',
            relatedFifoGuideDesc: 'FIFO収録の入門ガイド（欠損の可視化つき）',
            relatedStepAnalysisDesc: '歩容パラメーターの詳細可視化',
            relatedGaitReportDesc: '歩行計測レポートの自動生成',
            apiDocs: 'API ドキュメント',
        ...
        en: {
            ...
            navNext: 'Examples',
            navBuy: 'Buy',
            headerHomeTitle: 'Back to the ORPHE-INSOLE.js top page (examples index)',
            ...
            connectBody: 'After connection, the header shows measured frequency, <span class="badge bg-success">L</span>/<span class="badge bg-primary">R</span> badges based on mount position, and battery state. The left and right panels are also reordered to match the actual feet. Use the <i class="bi bi-gear"></i> gear icon to choose <strong>Raw Sensor Data / Step Analysis</strong>, switch Raw Data between <strong>Realtime / FIFO</strong>, and select the realtime packet format.',
            ...
            connectPrepItem3: 'To inspect pressure values, choose realtime mode 3/4 or FIFO from the gear menu.',
            modeDataHeader: 'Data',
            modeRateHeader: 'Rate',
            modeUseHeader: 'Use',
...
insole.gotConvertedGyro = function (gyro) {
  // {x, y, z}, unit: deg/s
            ...
            swapPitchRoll: 'Swap pitch / roll',
            swapPitchRollNote: 'Depending on the firmware / IMU mounting, pitch and roll can be assigned the other way around, so lifting the toe tilts the model sideways. Turn this on when the view does not match the physical device — it applies to both the 3D model and the Euler gauges. CSV recording keeps the raw SDK values.',
            statoEyebrow: 'ESTIMATED LOAD CENTER',
...
insole.gotEuler = function (euler) {
  // {pitch, roll, yaw}, unit: rad
            ...
            fifoTitle: 'Lossless Capture (FIFO)',
            fifoDesc: 'Pulls sensor data from the firmware buffer <strong>without packet loss</strong> (equivalent to <code>read_sensor_data_by_tokoroten_loop</code>). Dropped packets are re-requested automatically. Realtime display pauses during capture and is driven by the captured data instead. <strong>FIFO mode has no quaternion</strong> (gyro, accel, and 6-channel pressure only).',
            fifoStart: 'Start Capture',
            fifoStop: 'Stop Capture',
            fifoStartHtml: '<i class="bi bi-play-fill"></i> Start Capture',
            fifoStopHtml: '<i class="bi bi-stop-fill"></i> Stop Capture',
            fifoDownload: 'Save CSV',
            fifoAutoStop: 'Auto-stop on loss',
            fifoStatusStopped: '⚠ Auto-stopped on detected loss: {packets} packets / {dropped} lost — CSV is ready to save',
            fifoStatusIdle: 'Connect a device to enable',
            fifoStatusReady: 'Connected — ready to capture',
            fifoStatusPreparing: 'Preparing… (clearing buffer)',
            fifoCollecting: 'Capturing: {packets} packets / {seconds}s (lag {lag})',
            fifoLossWarning: '⚠ {dropped} lost — BLE could not keep up',
            fifoStatusDone: 'Done: {packets} packets / no loss — CSV is ready to save',
            fifoStatusDoneLoss: '⚠ Done: {packets} packets / {dropped} lost — CSV is ready to save',
            fifoDraining: 'Recovering after stop… ({seconds}s) — pulling outstanding serials from the FW buffer',
            fifoDrainNote: ' / {recovered} recovered after stop',
            fifoFailed: 'Capture failed (check the connection)',
            gaitTitle: 'Gait Analysis',
            gaitDesc: 'Receives the <strong>gait parameters</strong> the firmware computes in real time while walking (stride, stance/swing phase, foot-strike pattern, pronation, and more). Just subscribe to the dedicated characteristic notified at 50 Hz, and aggregated values arrive once per step (equivalent to <code>read_gait_analysis</code>). Walk around — walking in place works too.',
            gaitStart: 'Start',
            gaitStop: 'Stop',
            gaitStartHtml: '<i class="bi bi-play-fill"></i> Start',
            gaitStopHtml: '<i class="bi bi-stop-fill"></i> Stop',
            gaitDownload: 'Save CSV',
            gaitStatusIdle: 'Connect a device to enable',
            gaitStatusReady: 'Connected — walk to measure',
            gaitStatusRunning: 'Measuring… please walk ({steps} steps)',
            gaitStatusStopped: 'Done: {steps} steps — CSV is ready to save',
            gaitStatusError: 'Could not start Step Analysis ({codes}). Reconnect and try again',
            gaitWaiting: 'Waiting for steps… (walking in place works too)',
            gaitColGait: 'Gait',
            gaitColStride: 'Stride',
            gaitColCadence: 'Cadence',
            gaitColSpeed: 'Speed',
            gaitColStrike: 'Foot strike',
            gaitColPronation: 'Pronation',
            gaitColDistance: 'Distance',
            gaitColSteps: 'Steps',
            nextTitle: 'Next Steps (Examples)',
            ...
            nextStepAnalysisBody: 'Detailed visualization of FW-computed gait parameters',
            nextGaitReportBody: 'Automatic gait measurement reports',
            nextFifoGuideBody: 'Beginner guide to lossless FIFO capture',
            nextBalanceSwayBody: 'Center-of-pressure sway visualization and recording',
            nextSonifierBody: 'Motion sonification with Web Audio',
            nextTerminalBody: 'Debug raw data and BLE protocol behavior',
            nextAllExamples: 'See all examples (index)',
            backToIndex: 'Back to index (examples list)',
            relatedLabel: 'Related examples',
            relatedUdonDesc: 'Stomping game driven by pressure, no calibration needed',
            relatedBalanceTunerDesc: 'Sonify your load balance',
            relatedMusicShoeDesc: 'Play music with IMU gestures',
            relatedSonifierDesc: 'Motion sonification with Web Audio',
            relatedBalanceSwayDesc: 'Full center-of-pressure sway visualization and recording',
            relatedFifoGuideDesc: 'Beginner guide to FIFO capture with loss visualization',
            relatedStepAnalysisDesc: 'Detailed gait parameter visualization',
            relatedGaitReportDesc: 'Automatic gait measurement reports',
