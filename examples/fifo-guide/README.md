...
| 予定 | 収録serial | 収録スパン | maxLag | missing | dropped | 判定 |
|---|---:|---:|---:|---:|---:|---|
| 30秒 | 1325 | 26.5 s (88%) | 298 / 1500 | 0 | 0 | `PASS` |
| 60秒 | 2144 | 42.9 s (71%) | **1129 / 1500** | 4 | 4 | `FAIL` |

60秒では追従遅れが上限1500の75%まで達し、FWバッファから消えた4 serialが
`fw_nodata` として回復不能になりました。**lag が 1000 を超えたあたりから欠損が出始めます**。
30秒では lag 298 に収まり欠損0でした。これは環境依存の実測値であり、
どの長さでも「必ず欠損する / しない」を保証するものではありません。
