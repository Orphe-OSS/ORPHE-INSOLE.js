...
| オフセット（+21×n） | 型 | 内容 |
|---|---|---|
| 8,10,12,14 | int16 ×4 | quat w,x,y,z（Q14: /16384。単位クォータニオンのノルムは約1） |
...
（±250 / 500 / 1000 / 2000 dps）を使います。したがって、正規化値にレンジを単純に
掛けた値とは一致しません。

Euler 角は `gotQuat` と同じ成分を単位ノルムへ正規化してから計算します。これにより、
量子化誤差や将来の転送スケール変更があっても Euler 角の振幅が圧縮されません。

### v1.2.0 以前に記録した CSV の救済

v1.2.0 以前の SDK で記録し、各行の quaternion ノルムが約0.5になっている CSV は救済できます。
各行で `n = sqrt(w² + x² + y² + z²)` を計算し、`w/n, x/n, y/n, z/n` に置き換えてください。
`n ≈ 0.5` のデータでは4成分を一律2倍することと同じです。既存の `euler_*` 列は使わず、
修正後の quaternion から再計算してください。`n = 0` または非有限値の行は無効データとして除外します。
詳しい確認項目は [QUATERNION_FIX_VALIDATION.md](./QUATERNION_FIX_VALIDATION.md) を参照してください。
